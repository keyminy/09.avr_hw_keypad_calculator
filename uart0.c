...
ISR(USART0_RX_vect)
{
	...
	if (data == '\r' || data == '\n')
	{
		rx_buff[input_index][i] = '\0';   // 문장의 끝을 알리는  null을 insert
...
void pc_command_processing(void)
{
	...
	{
		printf("%s\n", rx_buff[output_index]);  // &rx_buff[front][0]
		if (strncmp(rx_buff[output_index],"led_all_on",strlen("led_all_on")) == 0)
		{
			PORTA=0xff;
		}
		else if (strncmp(rx_buff[output_index],"led_all_off",strlen("led_all_off")) == 0)
		{
			PORTA=0x00;
		}
		output_index++;
